...
class AttachmentsController extends taiga.Controller
    @.$inject = ["tgAttachmentsService", "$scope", "$tgResources"]

    constructor: (@attachmentsService, @scope, @rs) ->
        bindMethods(@)
    ...
    addAttachment: (attachment) ->
        attachmentRaw = attachment.toJS()

        promise = @attachmentsService.upload(attachmentRaw.file, @.objectId, @.projectId, @.type)

        promise.then (attachmentObj) ->
